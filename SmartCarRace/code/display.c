...
fifo_struct display_receiver_fifo;                                           // 定义摄像头接收数据fifo结构体
uint8 display_receiver_buffer[CAMERA_RECEIVER_BUFFER_SIZE];                  // 定义摄像头接收数据缓冲区
uint8 display_send_image_frame_header[4] = {0x00, 0xFF, 0x01, 0x01};         // 定义摄像头数据发送到上位机的帧头
